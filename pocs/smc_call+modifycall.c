...
void test(void *mem, long page_size, long evict_set)
{
    ...
    void(*func_evicts[L1I_SET_NUM][L1I_SET_SIZE]);
    ...
    for (int j = 0; j < L1I_SET_NUM; j++)
    {
        for (int i = 0; i < L1I_SET_SIZE; i++)
        {
            ...
            if (va_evict.split.index > j)
            {
                va_evict.split.tag++;
            }
            va_evict.split.tag = va_evict.split.tag + i;
            va_evict.split.index = j;
            func_evicts[j][i] = (void *)va_evict.address;
        }
    }
    va.address = (uint64_t)func_evicts[0][0];
    ...
    for (int j = 0; j < sum; j++)
    {
        ...
        for (int k = 0; k < L1I_SET_NUM; k++)
        {
            for (volatile int l = 0; l < L1I_SET_SIZE; l++)
            {
                func = func_list[k][l];
                result = func();
            }
        ...
        for (volatile int l = 0; l < L1I_SET_SIZE; l++)
        {
            volatile void (*func_evict)() = func_evicts[evict_set][l];
#if defined(__riscv)
            asm volatile(
                "lui x11, 0\n\t"
                "addi x11, %0, 0\n\t"
            ...
            func_evict();
            asm volatile("dsb st\n\tdsb sy\n\tisb\n\t");
            asm volatile("ic ivau, %0\n\tisb sy\n\tisb sy\n\t" ::"r"(func_list[evict_set][l]) : "memory");
#endif
        }
        ...
        {
            // for (size_t l = 0; l < L1I_SET_SIZE; l++)
            for (int l = L1I_SET_SIZE - 1; l >= 0; l--)
            {
                func = func_list[k][l];
#if defined(__x86_64__) //
                ((uint8_t *)func)[7] = 0x2;
#elif defined(__aarch64__) //
                ((uint8_t *)func)[5 + 4 * 8] = 0x8;
#elif defined(__riscv)     //
                ((uint8_t *)func)[6] = 0x25;
#endif
#if defined(__aarch64__)
                ...
                {
                    volatile int mix_i = k + ((i * 167) + 13) & 255;
                    if (mix_i % 2 == 0)
                    {
                        asm volatile("nop");
                    }
